...
def init_logger(name='plsc', log_file=None, log_level=logging.INFO):
...
def dict_format(d, float_placeholders="{:.5f}"):
    str_list = []
    for key in d:
        if isinstance(d[key], float):
            value = ("{}: " + float_placeholders).format(key, d[key])
        elif isinstance(d[key], AverageMeter):
            value = ("{}: " + float_placeholders).format(key, d[key].avg)
        else:
