...
class BatchOperator(object):
    ...
    def _mix_target(self, targets0, targets1, lam):
        ...
        if self.epsilon is not None:
            one_hots0 = np.array(
                F.label_smooth(
                    paddle.to_tensor(one_hots0), epsilon=self.epsilon))
        one_hots1 = self._one_hot(targets1)
        if self.epsilon is not None:
            one_hots1 = np.array(
                F.label_smooth(
                    paddle.to_tensor(one_hots1), epsilon=self.epsilon))
...
class Mixup(BatchOperator):
    ...
    def __init__(self, class_num, alpha: float=1., epsilon=None):
        ...
        """
        super(Mixup, self).__init__(epsilon=epsilon)
        if alpha <= 0:
            raise Exception(
...
class Cutmix(BatchOperator):
    ...
    def __init__(self, class_num, alpha=0.2, epsilon=None):
        ...
        Raises:
            Exception: The value of parameter is illegal.
        """
        super(Cutmix, self).__init__(epsilon=epsilon)
        if alpha <= 0:
            raise Exception(
...
class TransformOpSampler(object):
