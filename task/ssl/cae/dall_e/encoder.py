...
# Ref: https://github.com/openai/DALL-E/blob/master/dall_e/encoder.py
...
class Encoder(nn.Layer):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        make_conv = partial(Conv2D, requires_grad=self.requires_grad)
        make_blk = partial(
            EncoderBlock, n_layers=n_layers, requires_grad=self.requires_grad)

        self.blocks = nn.Sequential(
            ('input', make_conv(self.input_channels, 1 * self.n_hid, 7)),
            ('group_1', nn.Sequential(
                * [(f'block_{i + 1}', make_blk(1 * self.n_hid, 1 * self.n_hid))
                   for i in blk_range],
                ('pool', nn.MaxPool2D(kernel_size=2)), )),
            ('group_2', nn.Sequential(
                * [(f'block_{i + 1}', make_blk(1 * self.n_hid if i == 0 else
                                               2 * self.n_hid, 2 * self.n_hid))
                   for i in blk_range],
                ('pool', nn.MaxPool2D(kernel_size=2)), )),
            ('group_3', nn.Sequential(
                * [(f'block_{i + 1}', make_blk(2 * self.n_hid if i == 0 else
                                               4 * self.n_hid, 4 * self.n_hid))
                   for i in blk_range],
                ('pool', nn.MaxPool2D(kernel_size=2)), )),
            ('group_4', nn.Sequential(* [(f'block_{i + 1}', make_blk(
                4 * self.n_hid if i == 0 else 8 * self.n_hid, 8 * self.n_hid))
                                         for i in blk_range], )),
            ('output', nn.Sequential(
                ('relu', nn.ReLU()),
                ('conv', make_conv(
                    8 * self.n_hid, self.vocab_size, 1, use_float16=False)),
            )), )
